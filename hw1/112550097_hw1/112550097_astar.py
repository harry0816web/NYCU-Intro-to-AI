...
with open(heuristicFile, 'r') as f:
    ...
    header = next(reader, None)
    for row in reader:
        node, h = int(row[0]), float(row[1])
        heuristic[node] = h

def astar(start, end):
    # Begin your code (Part 4)
    pq = []
    heapq.heappush(pq, (heuristic[start],0, start, [start]))  # (cost, node, path)
    visited = set()
    while pq:
        # 每次取出最小cost
        heuristic_cost, cost, node, path = heapq.heappop(pq)
        if node in visited:
            continue
        visited.add(node)
        if node == end:
        ...
        for neighbor, neighbor_cost in graph.get(node, []):
            if neighbor not in visited:
                f = cost + neighbor_cost + heuristic[neighbor]
