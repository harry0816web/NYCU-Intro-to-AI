...
avg_true_mean = np.zeros(3)
max_avg_reward = np.zeros(3)
cnt = 0
for epsilon in epsilon_list:
    ...
    for time in range(exp_times):
        ...
        avg_true_mean[cnt] += env.true_mean[env.optimal_action]
    
    max_avg_reward[cnt] = np.max(avg_reward)
    avg_true_mean[cnt] /= exp_times
    cnt += 1
...
print("Max Avg Reward: ", max_avg_reward)
print("True Mean of Optimal Action: ", avg_true_mean)
